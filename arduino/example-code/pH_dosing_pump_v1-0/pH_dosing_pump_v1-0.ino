...
void loop() 
{
  ...
  {                                                      
    ...
    {                         
      ...
      if (PHAB.get_reading() <= 8)                             //test condition against pH reading
      {                            
        Serial.println("PH LEVEL LOW,PMP_UP = ON");
        PUMPA.send_cmd_with_num("d,", 0.5);                  //if condition is true, send command to turn on pump (called PMP_UP) and dispense pH up solution, in amounts of 0.5ml. Pump turns clockwise.
      }
      else                                                    //if condition is false, send command to turn off pump (called PMP_UP)
      {
        PUMPA.send_cmd("x");                                 
      }

      if (PHAB.get_reading() >= 8.5)                            //test condition against pH reading
      {                          
        Serial.println("PH LEVEL HIGH,PMP_DOWN = ON");
        PUMPB.send_cmd_with_num("d,", -0.5);               //if condition is true, send command to turn on pump (called PMP_DOWN) and dispense pH down solution, in amounts of 0.5ml. Pump turns counter-clockwise.
      }
      else                                                    //if condition is false, send command to turn off pump (called PMP_DOWN)
      {
        PUMPB.send_cmd("x");                               
      }
